...
if ($show == 1) {
	...
	if ($loop->have_posts()) {
        ...
        // Speed Bubble Frost colour
        $themo_frost_class = false;
        $themo_frost_contrast = get_post_meta($post->ID, $key.'_frost_contrast', true );
        if(isset($themo_frost_contrast)){
            if($themo_frost_contrast == 'white'){
                $themo_frost_class = 'white-frost';
            }elseif($themo_frost_contrast == 'black'){
                $themo_frost_class = 'black-frost';
            }
        }
		...
		while ($loop->have_posts()) {
			...
			if(($i == 1) || ($i-1) % 3 == 0){
				echo '<div class="row '.$themo_frost_class.'">';
			}
			...
			if(isset($metadata["_quote"]) && $metadata["_quote"] > ""){
				echo '<blockquote class="tweet blockquote-', $i, themo_return_entrance_animation_class($themo_enable_animate, $themo_animation_style, '#' . $key . ' .blockquote-' . $i), '">', $metadata["_quote"][0], '</blockquote>';
			}
			...
			if (get_the_title() > '') {
				if(isset($metadata['_title'][0]) && $metadata['_title'][0] > ""){
					$title_span = $metadata['_title'][0];
				}else{
					$title_span = false;
				}
				echo '<figcaption class="figcaption-', $i, themo_return_entrance_animation_class($themo_enable_animate, $themo_animation_style, '#' . $key . ' .figcaption-' . $i), '">', get_the_title(), '<span>'.$title_span.'</span></figcaption>';
			}
